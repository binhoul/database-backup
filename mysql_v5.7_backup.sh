...
function print_usage {
  ...
  echo
  echo "This script is used to backup mysql database."
  echo
  echo "Options:"
  echo
  echo -e "  --machine\t\tThe machine type of running mysql. Required. should be host or docker."
  echo -e "  --"
...
function  host_backup_auto {
  ...
  # 增量备份
    ...
    then
      innobackupex --defaults-file="${env_conf}" \
        --no-timestamp --user "${env_username}" \
        --host "${env_host}" \
        --port "${mysql_port}" \
        --password "${env_password}" \
        --incremental "${today_increment}" \
...
function main {
  ...
  if [[ ! -z ${env_input_port} ]];
  then
    declare -xr mysql_port=${env_input_port}
  else
    declare -xr mysql_port=${env_port}
  fi
  ...
  if [[ "${machine}" == "docker" ]]; then
    ...
    assert_is_installed innobackupex
  fi
